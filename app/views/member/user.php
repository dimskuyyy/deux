    ...
    <?php if (empty($data['getpost'])) {
        if (isset($_SESSION['UID'])) {
            if ($data['check'] == 'profil') {
                ...
                echo '<button class="account-follow-button"><a href="' . BASEURL . '/upload/" style="color:white">Upload Now</a></button>';
            }
        } else {
            echo '<div style="width:100%;display:flex;flex-direction:column;align-items:center;"><h2 style="text-align:center">Belum Ada Postingan</h2><br> ';
        }
        echo '</div>';
    } else {
    ?>
